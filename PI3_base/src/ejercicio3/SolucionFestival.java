...
public class SolucionFestival {
    ...
    private SolucionFestival(List<Integer> ls) {
    	...
    	for(int i=0; i < ls.size(); i++) {
    		if (ls.get(i) != 0) {
    			Integer TipoActual = i / DatosFestival.getNumAreas();
    			Integer AreaActual = i % DatosFestival.getNumAreas(); 
    			this.numAsignaciones ++;
    			this.solucion.put(i, ls.get(i));
    			this.costeTotal += ls.get(i) * DatosFestival.getCosteAsignacion(TipoActual, AreaActual);
    			this.unidadesTotales = this.unidadesTotales + ls.get(i);
    		}
    	}
    }
